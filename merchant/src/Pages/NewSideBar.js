...
const NewSideBar = () => {
  ...
  const handleLogout = async () => {
    try {
        const response = await fetch(`${backUrl}/merchant/mlogout`, {
            method: 'POST',
  ...
  return (
    <aside className="text-black  flex flex-col  fixed top-0 left-0 z-40 w-64 h-screen">
      ...
      <div
        ...
        } md:block h-full  px-3 space-y-2 font-medium py-4 overflow-y-auto bg-gray-50 DARK:bg-gray-800`}
      >
        {/* Dropdown */}
        <Link
          to="dashboard"
        ...
        <div>
          ...
          {dropdownState.product && (
            <div className="mt-2 pl-4">
              ...
              <hr className="h-px my-1 bg-gray-400 border-0" />
              <Link
                to="financial-pay-utr"
                className="block px-4 py-2 hover:bg-gray-200 text-sm"
              >
                Financial Payment UTR
              </Link>
              <hr className="h-px my-1 bg-gray-400 border-0" />
              <Link
                to="merchant-payment-recieved"
                className="block px-4 py-2 hover:bg-gray-200 text-sm"
              >
                Merchant Payment Recieved
              </Link>
            </div>
          )}
        </div>
        ...
        <div>
          <button
            ...
            className="w-full text-left px-4 py-2 mb-4 bg-gray-500 hover:bg-red-600 focus:outline-none"
          >
            Log Out
          </button>
        </div>
