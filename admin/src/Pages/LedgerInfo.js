...
const LedgerInfo = () => {
    ...
    useEffect(() => {
        axios.post(`${backUrl}/admin/ledgerinfo`, {Email},)
            .then((res) => {
                console.log(res, 'wallet data')
                console.log(res.data.Payinfo, "dd")
                setData(res.data.Payinfo ? res.data.Payinfo.reverse() : [] );
        ...
        <div className="min-h-screen flex mt-2 justify-center">
            <div className="bg-white rounded-lg shadow-lg p-2 w-full">
                ...
                <div className="mb-8 flex justify-between rounded-md px-4 py-2 bg-blue-900">
                    ...
                    <div>
                        <h2 className="font-semibold  text-gray-100">Balance:</h2>
                            ...
                            {data.length > 0 ? (
                                data.map((item, index) => (
                                    <tr key={index} class={` ${item.PAmount ? "bg-green-100" : "bg-white"} border-b DARKK:bg-gray-800 DARKK:border-gray-700 border-gray-200`}>
                                        ...
                                        <td class={`px-6 py-4 ${item.PAmount ? "text-green-500" : ""}`}>
                                        {item.PAmount }
                                        </td>
                                        <td class="px-6 py-4">
                                            {item.RunningBalance}
